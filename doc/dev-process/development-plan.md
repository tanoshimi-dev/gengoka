...
Step 4: アカウントリンク機能
         ...
         └─ 複数プロバイダーリンク対応
              │
Step 5: テスト・検証
         ├─ Backend ユニットテスト（wiremock使用、21テスト） ─── 完了
         ├─ 各プロバイダーでの新規登録フロー ─── Phase 2へ移行
         ├─ 既存ユーザーとのアカウントリンク ─── Phase 2 Step 2へ移行
         ├─ エッジケース（メール重複、トークン期限切れ等） ─── Phase 2 Step 2へ移行
         └─ UI/UXレビュー ─── Phase 2 Step 3-4へ移行
```
...
Step 1: Backend テスト基盤構築
         ...
         └─ 認証フローのE2Eテスト
              │
Step 2: Backend テスト拡充（Phase 1 Step 5 残件含む）
         ├─ アカウントリンク統合テスト（DB必要）
         ├─ エッジケーステスト（メール重複、プロバイダー重複等）
         ├─ ソーシャル認証テスト（モック使用）
         ├─ チャレンジ/回答ハンドラーテスト
         └─ ユーザー/フィード/ランキングテスト
              │
Step 3: iOS テスト基盤構築（Phase 1 Step 5 残件含む）
         ├─ テストターゲット追加
         ├─ APIClient Protocol化（DI対応）
         ├─ AuthService ユニットテスト
         ├─ SocialAuthService ユニットテスト（Google/Apple/LINE）
         ├─ 認証UIテスト
         └─ ソーシャルログインUI/UXレビュー
              │
Step 4: Android テスト基盤構築（Phase 1 Step 5 残件含む）
         ├─ テスト依存関係追加
         ├─ AuthRepository ユニットテスト
         ├─ AuthViewModel ユニットテスト（ソーシャル認証含む）
         ├─ 認証画面 Compose UIテスト
         └─ ソーシャルログインUI/UXレビュー
              │
Step 5: テスト拡充 + CI連携
